class UsersController < ApplicationController
  ...
  def show
    ...
    @links = Link.find_by_sql( ["SELECT IFNULL(expanded_url, url) AS url, page_title, COUNT(*) AS n FROM links JOIN linkages ON links.id = link_id JOIN friendships f ON f.friend_id = linkages.user_id WHERE f.user_id = ? GROUP BY IFNULL(expanded_url, url) ORDER BY COUNT(*) DESC LIMIT 30", @user.id ] )
